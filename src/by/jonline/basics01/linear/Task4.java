...
public class Task4 {
	...
	// Сделаем допущение, что числа вида 0nn.0dd будут преобразовываться к виду dd.0nn
	public static void main(String[] args) {
		// число запросим у пользователя через консоль.
		Scanner scanner = new Scanner(System.in);
		String input;
		float result;
		// используем регулярное выражение как паттерн для проверки правильности ввода пользователя.
		...
		do {
			...
			if (input.matches(pattern)) {//проверяем формат ввода
				result=Float.parseFloat(input.substring(4, 7)+"."+input.substring(0,3));//составляем новое чило и записываем его в переменную result
